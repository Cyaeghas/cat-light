...
The least risky first tray target is Windows Win32 tray + localhost dashboard. A cross-platform tray can come later through Qt or wxWidgets.
...
- Optional SQLite through `CAT_LIGHT_ENABLE_SQLITE`.
...
- Optional tray target: later CMake option, for example `CAT_LIGHT_ENABLE_TRAY=ON`.

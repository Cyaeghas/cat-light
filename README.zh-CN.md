...
- 增加本地数据库 migration/version metadata。
- 在 provider 日志安全暴露 per-tool delta 时改进 token 归因。
- 改进 dashboard 的趋势、上下文和会话历史图表。

v0.4 桌面体验：

- 跨平台托盘或浮标 UI。
- 改进 Windows 托盘图标状态渲染和通知。
- 改进 installer 或 portable-app 打包体验。
- 模型价格元数据和估算成本视图。
